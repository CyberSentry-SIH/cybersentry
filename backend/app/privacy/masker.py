...
def mask_indian_pii(text: str) -> str:
    ...
    if not text:
        return ""
    text = str(text)  # Coerce None/int to string safely

    masked = text

    # UPI ID: alphanumeric or mobile@bankcode (e.g., user@okicici, 9876543210@ybl)
    masked = re.sub(
        r'\b[A-Za-z0-9._-]+@(?:ok(?:icici|axis|sbi|hdfc)|ybl|paytm|upi|apl|ibl|axl|sbi|hdfcbank|icici|kotak|barodampay|waaxis|wahdfc|waicici|wasbi)\b',
        '[REDACTED_UPI]',
        masked,
        flags=re.IGNORECASE
    )
