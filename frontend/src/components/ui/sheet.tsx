...
const sheetVariants = cva(
  "fixed z-50 flex flex-col gap-0 border-border-default bg-bg-surface shadow-xl transition ease-in-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
        right:
...
function SheetContent({
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        <DialogPrimitive.Close className="absolute right-3 top-3 rounded-sm text-text-muted transition-colors hover:text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-primary">
          <X className="h-4 w-4" />
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "flex flex-col gap-1 border-b border-border-default px-4 py-3",
        className
      )}
      {...props}
    />
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "mt-auto flex items-center justify-end gap-2 border-t border-border-default px-4 py-3",
        className
      )}
      {...props}
    />
